        ...
        const fetchData = async () => {
          try {
            ...
            const [originResponse] = await Promise.all([
                axios.get(`https://eshwar-backend.azurewebsites.net/api/v1/employeeManager/origin/${employeeId}`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('https://eshwar-backend.azurewebsites.net/api/leaves/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            console.log(formData);
            const url = isEditing
                ? `https://eshwar-backend.azurewebsites.net/api/leaves/update/${formData.id}`
                : `https://eshwar-backend.azurewebsites.net/api/leaves/submit`;
