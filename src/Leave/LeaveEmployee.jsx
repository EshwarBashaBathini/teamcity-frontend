...
export default function LeaveEmployee() {
  ...
  if (!employeeId) {
    ...
    return;
      ...
      try {
        ...
        console.log(token)
        const response = await axios.get(`https://eshwar-backend.azurewebsites.net/api/leaves/employee/${employeeId}`, {
          headers:{
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token')
      await axios.delete(`https://eshwar-backend.azurewebsites.net/api/leaves/delete/${deleteRequestId}`, {
        method:'GET',
        headers:{
          'Authorization' : `Bearer ${token}`,
          'Content-Type' : 'application/json'
