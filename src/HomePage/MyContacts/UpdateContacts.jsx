...
const UpdateContacts = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          ...
          try {
            const response = await axios.get(`https://eshwar-backend.azurewebsites.net/apis/employees/contacts/contacts/${contactId}`,{
                headers: {
    ...
    const handleSubmit = async(e) => {
        e.preventDefault();
        setIsAdding(true);
        await axios.put(`https://eshwar-backend.azurewebsites.net/apis/employees/contacts/contacts/${contactId}`, formData,{
            headers: {
              "Authorization": `Bearer ${token}`  // Add the token to the Authorization header
            }
          });
