...
const EmployeeHomePage = ({ submissions, setSubmissions }) => {
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      try {
        let url = `https://eshwar-backend.azurewebsites.net/api/timesheets/list/${employeeId}`;
  ...
  const handleDeleteTimesheet = async () => {
    try {
      await axios.delete(`https://eshwar-backend.azurewebsites.net/api/timesheets/delete/${selectedSubmissionId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
